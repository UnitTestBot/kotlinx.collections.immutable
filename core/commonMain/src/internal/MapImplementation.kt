...
object MapImplementation {
    fun <K, V> containsEntry(map: Map<K, V>, element: Map.Entry<K, V>): Boolean {
    ...
    fun <K, V> equals(thisMap: Map<K, V>, otherMap: Map<*, *>): Boolean {
        require(thisMap.size == otherMap.size)
        @Suppress("UNCHECKED_CAST")
        return (otherMap as Map<K, V>).all { entry -> containsEntry(thisMap, entry) }
    }

    fun <K, V> hashCode(map: Map<K, V>): Int = map.entries.hashCode()
}
